...
Se contó con asistencia de inteligencia artificial de manera transversal durante todas las etapas del desarrollo: interpretación del enunciado, organización del repositorio, implementación inicial, selección y análisis de series, construcción de verificaciones, diagnóstico de resultados, documentación y revisión del trabajo.

La estrategia de uso de la IA se orientó a aprovechar sus capacidades sin convertirla en un sustituto del proceso de aprendizaje. Sus objetivos fueron optimizar el trabajo iterativo, favorecer el pensamiento crítico, fortalecer el pensamiento algorítmico y profundizar y consolidar los conocimientos específicos de Series de Tiempo. Por ello, las respuestas generadas se trataron como propuestas que debían ejecutarse, contrastarse con las notas de clase, cuestionarse y explicarse.

El proceso comenzó solicitando una estructura de carpetas y archivos compatible con la especificación del profesor, junto con los contratos y declaraciones de las funciones requeridas. En esa primera etapa la IA también produjo primeras implementaciones completas; por tanto, su participación no se limitó a crear archivos vacíos o encabezados. A partir de esa base, el trabajo se organizó como una revisión función por función para comprender los datos de entrada, el algoritmo, las inicializaciones, las salidas, los casos límite y las comprobaciones numéricas de cada método.

La verificación realizada incluyó: contraste de fórmulas con las clases 1–4; ejecución completa desde la raíz; pruebas de dimensiones y mensajes de error; igualdad de la ACF manual con `stats::acf` a menos de `1e-12`; igualdad de Ljung–Box manual con `stats::Box.test`; comprobación de SES mediante su forma ponderada; y revisión de métricas sobre conjuntos separados de entrenamiento y validación. La serie del ejemplo exponencial se cambió después de comparar cuantitativamente distintas alternativas.

El autor conserva la responsabilidad académica sobre el resultado: revisar el código, reconstruir y explicar los algoritmos, identificar las limitaciones de los modelos y justificar las decisiones antes de presentar el trabajo.
